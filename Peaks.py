...
def peaks(sig1):
    ...
    sig1 = np.array(sig1)
    indices = indexes(sig1, thres=0.01,
                      min_dist=4)  # return the indeces of the peaks found in the array, with threshold, 0.01 and a minimun distance: 1 sec = 4 samples
    return (indices)
...
def num_of_peaks(beg, en, time, sig1):  # sig1 and sig2 should be the normalized signal
    ...
    for i in range(len(sess)):
        ...
        nmpeaks.append(
            len(peaks(sig1[indices1[0]:indices1[1]])))  # pearson correlation between the two signals in each timeslot
